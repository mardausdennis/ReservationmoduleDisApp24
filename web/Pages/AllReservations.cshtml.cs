...
public class AllReservationsModel : PageModel
{
    ...
    public async Task<IActionResult> OnGet()
    {
        ...
        PastReservations = Reservations.Where(r => DateTime.ParseExact(r.Date, "dd.MM.yyyy", CultureInfo.InvariantCulture).Date < currentTime.Date)
        ...
        UnconfirmedPastReservations = Reservations.Where(r => r.Status == "Pending" && DateTime.ParseExact(r.Date, "dd.MM.yyyy", CultureInfo.InvariantCulture).Date < currentTime.Date)
           .OrderByDescending(r => DateTime.ParseExact(r.Date, "dd.MM.yyyy", CultureInfo.InvariantCulture))
           .ThenByDescending(r => TimeSpan.ParseExact(r.TimeSlot.Split('-')[0], @"hh\:mm", CultureInfo.InvariantCulture))
           .ToList();
